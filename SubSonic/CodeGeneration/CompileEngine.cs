...
namespace SubSonic
{
    ...
    public class TurboCompiler
    {
        ...
        public void SaveTempCode(string fileName, string txt)
        {
            using (System.IO.StreamWriter writer = new System.IO.StreamWriter("C:\\" + fileName + ".cs", true))
            {
        ...
        public void Run()
        {
            ...
            if (templateCount > 0)
            {
                ...
                if (results.Errors.Count > 0 || results.CompiledAssembly == null)
                {
                    ...
                    {
                        string errTxt = errorMessage;
                        foreach (CompilerError error in results.Errors)
                            errTxt = string.Concat(errTxt, "\n", error.ErrorText);
        ...
        /// </summary>
        /// <param name="assembly">The assembly.</param>
        private void CallEntry(Assembly assembly, TurboTemplate template)
        {
            Module mod = assembly.GetModules(false)[0];
            Type type = mod.GetType(template.GeneratedRenderType);
            if (type != null)
            {
                MethodInfo mi = type.GetMethod(template.EntryPoint, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
                if (mi != null)
                {
                    StringBuilder returnText = new StringBuilder();

                    string resultMessage = String.Format("Successfully generated {0}", template.TemplateName);
                    if (template.AddUsingBlock)
                    {
                        Utility.WriteTrace("Adding Custom Using Blocks");
                        returnText.Append(template.RenderLanguage.DefaultUsingStatements);
                        returnText.Append(template.CustomUsingBlock);
                    }
                    try
                    {
                        returnText.Append((string)mi.Invoke(null, null));
                    ...
                    {
                        resultMessage = "An Exception occured in template " + template.TemplateName + ": " + ex.Message;
                    }

                    template.FinalCode = ScrubOutput(returnText.ToString(), template.RenderLanguage);
                    Utility.WriteTrace(resultMessage);
                }
            }
        }

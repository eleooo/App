...
namespace SubSonic
{
    ...
    public class Sql2005Generator : ANSISqlGenerator
    {
        ...
        private const string PAGIND_SQL_CUSTOME =
            @"
            SELECT {8} FROM (
            SELECT ROW_NUMBER() OVER ({1}) AS Row, 
            r.* FROM (
            ...
            {4}
            {5}
            ) as r) AS PagedResults
            WHERE  Row >= {6} AND Row <= {7}";
        //columnList, orderby, fromLine, joins, wheres, groupby, pageStart, pageEnd, top
        /// <summary>
        ...
        {
            string idColumn = GetSelectColumns( )[0].Trim( );
            if (idColumn.Contains(" "))
                idColumn = idColumn.Split(new char[] { ' ' })[0];
            else if (query.IsDistinct && idColumn.Contains("."))
                idColumn = idColumn.Substring(idColumn.LastIndexOf(".") + 1);
            return idColumn;
        }
        /// <summary>
        ...
        public override string BuildPagedSelectStatement( )
        {
            ...
            string orderby = !string.IsNullOrEmpty(qry.DefaultPagingSort) ? String.Concat(SqlFragment.ORDER_BY, qry.DefaultPagingSort) : GenerateOrderBy( );
            string groupby = string.Empty;

            if (String.IsNullOrEmpty(orderby.Trim( )))
            {
                orderby = String.Concat(SqlFragment.ORDER_BY, GetIdColumn( ));
            }
            ...
            if (!String.IsNullOrEmpty(qry.TopSpec))
            {
                ...
                top = String.Concat(String.Format("{0} {1}", qry.TopSpec, top), " ", columnList);
            }
            ...
            if (qry.IsDistinct)
                sql = string.Format(PAGIND_SQL_CUSTOME, columnList, orderby, fromLine, joins, wheres, groupby, pageStart, pageEnd, top);
            else
                sql = string.Format(PAGING_SQL, columnList, orderby, fromLine, joins, wheres, groupby, pageStart, pageEnd, top);
            return sql;
        }
    }
}

...
namespace SubSonic
{
    ...
    public class ANSISqlGenerator : ISqlGenerator
    {
        ...
        public virtual string GenerateJoins()
        {
            ...
            if(query.Joins.Count > 0)
            {
                ...
                foreach(Join j in query.Joins)
                {
                    ...
                    foreach (string joinExpression in j.JoinExpressions)
                    {
                        sb.Append(string.Format(" {0} ", joinExpression));
                    }
        ...
        private void BuildConstraintSQL(ref string constraintOperator, StringBuilder sb, bool isFirst, ref bool expressionIsOpen, Constraint c)
        {
            ...
            if(c.ConstructionFragment == c.ColumnName && c.ConstructionFragment != "##")
            {
                ...
                if (c.Column != null && col != null)
                {
                    ...
                    foundColumn = true;
                    c.ParameterName = query.Provider.FormatParameterNameForSQL(col.ParameterName, query.Constraints.IndexOf(c));
                }
                else
                {
                    if(col != null)
            ...
            if(!foundColumn && c.ConstructionFragment != "##")
            {
                ...
                {
                    c.ParameterName = query.Provider.FormatParameterNameForSQL(rawColumnName, query.Constraints.IndexOf(c));
                    columnName = c.ConstructionFragment;
                }
            }
        ...
        public virtual string GenerateWhere()
        {
            ...
            if (query.ConstraintExpressions.Count > 0)
            {
                foreach (string exp in query.ConstraintExpressions)
                    sb.AppendLine(exp);
                result = sb.ToString();
            }
        ...
        public virtual string GetCountSelect()
        {
            ...
            if ((query.Aggregates.Count > 0 || query.IsDistinct || IsAggregated) && query.OrderBys.Count == 0)
            {
        ...
        public bool IsAggregated
        {
            get
            {
                string pattern = @"group\s+by";
                System.Text.RegularExpressions.Regex regex = new System.Text.RegularExpressions.Regex(pattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                foreach (string expression in query.ConstraintExpressions)
                {

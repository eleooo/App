...
namespace SubSonic
{
    ...
    public class SQLiteDataProvider : DataProvider
    {
        ...
        internal SQLiteDataProvider(System.Collections.Specialized.NameValueCollection config, params object[] ds)
            : base(config, ds)
        {
            if (ds != null)
            {
                ...
                {
                    DataTable dt = ds[i] as DataTable;
                    NamedList<string> lst;
                    if (dt != null)
                    {
                        if (Utility.Compare(dt.TableName, "AllColumns"))
                            _columns = dt;
                        else if (Utility.Compare(dt.TableName, "AllForeignKeys"))
                            _foreignkeys = dt;
                        else if (Utility.Compare(dt.TableName, "AllTypes"))
                            _types = dt;
                    }
            ...
            configs = null;
            configs = new System.Collections.Specialized.NameValueCollection(config);
            Initialize();
        }
            ...
            get
            {
                if (_foreignkeys == null)
                {
                    using (SQLiteConnection con = (SQLiteConnection)CreateConnection())
                    {
                        ...
                        _foreignkeys.TableName = "AllForeignKeys";
                    }
                }
        ...
        public Type GetType(string sqliteType)
        {
            ...
            DataRow[] type = AllTypes.Select(string.Format("Typename = '{0}'", sqliteType));
            ...
            if (TableNames == null || TableNames.Length == 0)
            {
                ...
                using (SQLiteConnection conn = (SQLiteConnection)CreateConnection())
                {
                    using (SQLiteCommand cmd = new SQLiteCommand(sql, conn))
                    {
                        ...
                        {
                            NamedList<string> _tables = new NamedList<string>("AllTables");

                            while (rdr.Read())
                                _tables.Add(rdr[0].ToString());
                            TableNames = _tables.ToArray();
                        }
                    }
                }
            }
        ...
        public override string[] GetViewNameList()
        {
            if (ViewNames == null || ViewNames.Length == 0)
            {
                using (SQLiteConnection conn = (SQLiteConnection)CreateConnection())
                {
                    ...
                    ViewNames = new string[views.Rows.Count];
                    for (int n = 0; n < ViewNames.Length; n++)
                        ViewNames[n] = views.Rows[n]["TABLE_NAME"].ToString();
                    Array.Sort(ViewNames);
                }
            }
            return ViewNames;

...
FROM  INFORMATION_SCHEMA.COLUMNS";
        ...
        private readonly DataSet dsColumns;
        private readonly DataSet dsExtendedProperties;
        private readonly DataSet dsFK;
        private readonly DataSet dsIndex;
        private readonly DataSet dsManyToManyCheck;
        private readonly DataSet dsManyToManyMap;
        private readonly DataSet dsPK;
        private DataTable dtParamSql;

        public SqlCEProvider( )
        {
            dsColumns = new DataSet("dsColumns");
            dsExtendedProperties = new DataSet("dsExtendedProperties");
            dsFK = new DataSet("dsFK");
            dsIndex = new DataSet("dsIndex");
            dsManyToManyCheck = new DataSet("dsManyToManyCheck");
            dsManyToManyMap = new DataSet("dsManyToManyMap");
            dsPK = new DataSet("dsPK");
        }

        internal SqlCEProvider(System.Collections.Specialized.NameValueCollection config, params object[] ds)
        {
            if (ds != null)
            {
                for (int i = 0; i < ds.Length; i++)
                {
                    DataSet dataSet = ds[i] as DataSet;
                    NamedList<string> lst;
                    if (dataSet != null)
                    {
                        if (Utility.Compare(dataSet.DataSetName, "dsColumns"))
                            dsColumns = dataSet;
                        else if (Utility.Compare(dataSet.DataSetName, "dsExtendedProperties"))
                            dsExtendedProperties = dataSet;
                        else if (Utility.Compare(dataSet.DataSetName, "dsFK"))
                            dsFK = dataSet;
                        else if (Utility.Compare(dataSet.DataSetName, "dsIndex"))
                            dsIndex = dataSet;
                        else if (Utility.Compare(dataSet.DataSetName, "dsManyToManyCheck"))
                            dsManyToManyCheck = dataSet;
                        else if (Utility.Compare(dataSet.DataSetName, "dsManyToManyMap"))
                            dsManyToManyMap = dataSet;
                        else if (Utility.Compare(dataSet.DataSetName, "dsPK"))
                            dsPK = dataSet;
                    }
                    else if ((lst = ds[i] as NamedList<string>) != null)
                    {
                        if (Utility.Compare(lst.Name, "AllTables"))
                            TableNames = lst.ToArray( );
                        else if (Utility.Compare(lst.Name, "AllViews"))
                            ViewNames = lst.ToArray( );
                    }
                }
            }
            if (dsColumns == null)
                dsColumns = new DataSet("dsColumns");
            if (dsExtendedProperties == null)
                dsExtendedProperties = new DataSet("dsExtendedProperties");
            if (dsFK == null)
                dsFK = new DataSet("dsFK");
            if (dsIndex == null)
                dsIndex = new DataSet("dsIndex");
            if (dsManyToManyCheck == null)
                dsManyToManyCheck = new DataSet("dsManyToManyCheck");
            if (dsManyToManyMap == null)
                dsManyToManyMap = new DataSet("dsManyToManyMap");
            if (dsPK == null)
                dsPK = new DataSet("dsPK");
            if (config != null)
            {
                base.configs = new System.Collections.Specialized.NameValueCollection(config);
                base.Initialize( );
            }
        }
        ...
        public override System.Collections.Generic.List<object> GetDataBaseAllSchema( )
        {
            List<object> lst = new List<object>( );
            lst.Add(dsColumns);
            lst.Add(dsExtendedProperties);
            lst.Add(dsFK);
            lst.Add(dsIndex);
            lst.Add(dsManyToManyCheck);
            lst.Add(dsManyToManyMap);
            lst.Add(dsPK);
            NamedList<string> t = new NamedList<string>("AllTables");
            t.AddRange(GetTableNameList( ));
            lst.Add(t);
            NamedList<string> v = new NamedList<string>("AllViews");
            t.AddRange(GetViewNameList( ));
            lst.Add(v);
            return lst;
        }

...
namespace SubSonic
{
    public class AccessDataProvider : DataProvider
    {
        ...
        private readonly DataSet dsSPParams;
        private readonly NamedListDict arSP;
        private readonly NamedListDict arViews;
        private readonly NamedListDict arTables;
        ...
        public AccessDataProvider( )
        {
            ...
            arSP = new NamedListDict("arSP");
            arViews = new NamedListDict("arViews");
            arTables = new NamedListDict("arTables");
        ...
        internal AccessDataProvider(NameValueCollection config, params object[] ds)
        {
            if (ds != null)
            {
                for (int i = 0; i < ds.Length; i++)
                {
                    ...
                    else if ((lstdict = ds[i] as NamedListDict) != null)
                    {
                        if (Utility.Compare(lstdict.Name, "arSP"))
                            arSP = lstdict;
                        else if (Utility.Compare(lstdict.Name, "arViews"))
                            arViews = lstdict;
                        else if (Utility.Compare(lstdict.Name, "arTables"))
                            arTables = lstdict;
                    }
                    else if ((lst = ds[i] as NamedList<string>) != null)
                    {
                        if (Utility.Compare(lst.Name, "AllTable"))
        ...
        public override string FormatParameterNameForSQL(string columnName, int index)
        {
            return FormatParameterNameForSQL(base.FormatParameterNameForSQL(columnName, index));
        }
        /// <summary>

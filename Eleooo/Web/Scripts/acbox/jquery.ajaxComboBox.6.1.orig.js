	...
	function individual(_jqobj, _source, _options) {
		...
		function initElements(_jqobj) {
			...
			if (Opt.plugin_type == 'combobox') {
				elems.button = $('<span>').addClass(Cls.button);
			...
			//ComboBoxの幅
			if (Opt.plugin_type == 'combobox') {
				//$(elems.container).width($(elems.combo_input).outerWidth() + $(elems.button).outerWidth());
				//ボタンの高さ
		...
		function calcWidthResults() {
			...
			//ComboBoxの幅
			if (Opt.plugin_type == 'combobox') {
				var w = $(Elem.combo_input).width() + $(Elem.button).width();
			} else {

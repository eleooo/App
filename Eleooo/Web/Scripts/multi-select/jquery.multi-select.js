    ...
    MultiSelect.prototype = {
        ...
        'select': function (value, method) {
            ...
            $.each(value, function (i, val) {
                if ($.inArray(val, that.selectedVal) > -1) return;
                var id = that.sanitize(val, that.sanitizeRegexp);
        ...
        'deselect': function (value) {
            ...
            $.each(value, function (i, val) {
                ...
                that.$selectableUl.find("#" + id + "-selectable").show();
                ssLi.remove();
                if (ssOptGroup.find("li :visible").length <= 1)
                    ssOptGroup.remove();
                var pos = $.inArray(val, that.selectedVal);
                if (pos > -1)
        ...
        isDomNode: function (attr) {
            return attr && attr.name.indexOf("data-")>-1;
//            return (
//        attr &&
//        typeof attr === "object" &&

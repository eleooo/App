<%@ Page Title="提供商家优惠" Language="C#" MasterPageFile="~/MasterPage/MainMaster.Master" AutoEventWireup="true" CodeBehind="CompanyItemEdit.aspx.cs" Inherits="Eleooo.Web.Admin.CompanyItemEdit" %>

<%@ Register Src="~/Controls/UcPagePosition.ascx" TagName="UcPagePosition" TagPrefix="uc" %><%@ Register Src="~/Controls/UcFormView.ascx" TagName="UcFormView" TagPrefix="uc" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <link href="/Scripts/jquery.tipswindown/jquery.tipswindown.css" rel="stylesheet" type="text/css" />
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //bind change event
            bindChange("CompanyTel", function () {
                var phone = $("#CompanyTel").val();
                if (phone && phone.length == 11) {
                    $.ajax({ url: "/Public/RestHandler.ashx/Area/GetMansionArea",
                        type: "POST",
                        dataType: "json",
                        data: { phone: phone },
                        success: function (result) {
                            if (result.code == 0) {
                                $('#slMulitSelector3_TextArea').areaext().addTagsByDepth(result.data);
                                if (result.data.length > 0) {
                                    var depth = [];
                                    $.each(result.data[result.data.length - 1].depth.split("/"), function (i, d) {
                                        if (d && d.length > 0)
                                            depth.push(d);
                                    });
                                    $.fn.regMulitSelector({
                                        sid: 'slMulitSelector3_0,slMulitSelector3_1,slMulitSelector3_2',
                                        dv: depth.join(",")
                                    });
                                }
                            }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderMain" runat="Server">
    ...
    <uc:UcFormView ID="formView" runat="server" /><asp:HiddenField ID="hdfItemInfo" runat="server" />

...
namespace Eleooo.Web.Company
{
    public partial class MemberList : ActionPage
    {
        ...
        protected override void On_ActionQuery(object sender, EventArgs e)
        {
            ...
            var query = DB.Select(Utilities.GetTableColumns(SysMember.Schema, SysMember.MemberPhoneNumberColumn),
                                  CompanyBLL.RenderUserPhone(VPayment.PaymentMemberIDColumn, VPayment.PaymentCompanyIDColumn, SysMember.MemberPhoneNumberColumn, SysMember.MemberCompanyIDColumn),
                                  SysCompany.Columns.CompanyName).Distinct( )
                                    .From<SysMember>( )
                                    .InnerJoin(VPayment.PaymentMemberIDColumn, SysMember.IdColumn)
                                    .InnerJoin(SysCompany.IdColumn, SysMember.MemberCompanyIDColumn)
                                    .Where(VPayment.PaymentDateColumn).IsBetweenAnd(dtBegin, dtEnd)
                                    .ConstraintExpression(CompanyBLL.RenderIsOwner(VPayment.PaymentMemberIDColumn, VPayment.PaymentCompanyIDColumn, MemberFilter.Outer))
                                    .And(VPayment.PaymentCompanyIDColumn).IsEqualTo(CurrentUser.CompanyId)
            ...
            gridView.AddShowColumn(SysMember.MemberFullnameColumn)
                    ...
                    .AddCustomColumn(SysCompanyMemberGrade.Columns.GradeName, ResBLL.GetColumnRes(SysCompanyMemberGrade.GradeNameColumn));
        ...
        string gridView_OnDataBindColumn(string column, System.Data.DataRow rowData, ref bool isRenderedCell)
        {
            ...
            switch (column)
            {
                ...
                case "MemberStatus":
                    ...
                    break;
                case ACTION_COLUMN:
                    result = string.Concat("[", GetEditDlgHtml(rowData, "详细", 0), "]");
                    break;
                case "GradeName":
                    string grade = GradeBLL.GetUserCurrentGrade(CurrentUser.CompanyId.Value, Utilities.ToInt(rowData[SysMember.Columns.Id]));
